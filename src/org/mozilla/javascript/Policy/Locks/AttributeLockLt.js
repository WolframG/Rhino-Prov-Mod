...
var AttributeLockLt = function(lock) {
    // call the super class constructor
    AttributeLockLt.super_.call(this, lock);
};

Lock.registerLock("hasAttributeLt", AttributeLockLt);

system.inherits(AttributeLockLt, Lock);

AttributeLockLt.prototype.copy = function() {
    var c = new AttributeLockLt(this);
    return c;
};

AttributeLockLt.prototype.getAttribute = function(attributes) {
...
AttributeLockLt.prototype.handleUser = function(context) {
...
AttributeLockLt.prototype.handleSO = function(context) {
    if(context.isStatic) {
        throw new Error("Not supported yet");
    } else {
...
AttributeLockLt.prototype.handleSU = function(context) {
    if(context.isStatic) {
        throw new Error("Not supported yet");
    } else {
...
AttributeLockLt.prototype.handleMsg = function(context) {
    if(context.isStatic) {
        throw new Error("Not supported yet");
    } else {
...
// TODO ENSURE SIMILAR TO REPUTATION THAT REGULAR NODES DO NOT CREATE EXCEPTIONS

AttributeLockLt.prototype.handleNode = function(context) {
    var currentIDMInfo = null;
        ...
        if(context.subject && context.subject.data) {
            if(context.subject.data.instanceId) {
                if(context.subject.data.idm) {
                    ...
                    throw new Error("Invalid subject context format (missing idm).");
                }
            } else {
                return { result : true, conditional : true };
            }
...
//
//    * context.subject.data.idm - reference to idm to get info about the embedding application
AttributeLockLt.prototype.handleApp = function(context) {
    var currentIDMInfo = null;
    ...
    if(context.isStatic) {
        ...
        if(context.subject && context.subject.data) {
            ...
            throw new Error("Invalid subject context format.");
        }
...
AttributeLockLt.prototype.isOpen = function(context) {
    if(context && context.subject) {
        ...
        throw new Error("AttributeLockLt: Require context information to evaluate lock.");
    }
};

AttributeLockLt.prototype.lub = function(lock) {
    if(this.eq(lock))
...
AttributeLockLt.prototype.le = function(lock) {
    if(this.eq(lock))
        return true;
    else { 
...
if(global && typeof print !== "function")
    module.exports = AttributeLockLt;
